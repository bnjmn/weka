...
public class NaiveBayesMultinomialText extends AbstractClassifier implements
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    for (int i = 0; i < data.numClasses(); i++) {
      ...
      // this needs to be updated for laplace correction every time we see a new word (attribute)
      m_wordsPerClass[i] = 0; 
    }
  ...
  protected void updateClassifier(Instance instance, 
      boolean updateDictionary) throws Exception {
    ...
    for (int i = 0; i < m_data.numClasses(); i++) {
      ...
      if (m_normalize) {
        for (Map.Entry<String, Count> feature : m_inputVector.entrySet()) {
          ...
          // only normalize with respect to those words that we've seen during training
...
/*        if (dictCount != null) {
          System.out.println(dictCount.m_count);
        } else {
  ...
  protected void tokenizeInstance(Instance instance, boolean updateDictionary) {
    ...
    if (updateDictionary) {
      ...
      for (Map.Entry<String, Count> feature : m_inputVector.entrySet()) {
        ...
        Count dictCount = dictForClass.get(word);
/*        if (dictCount == null) {
          dictForClass.put(word, new Count(m_leplace + freq));
          m_wordsPerClass[classValue] += (m_leplace + freq);
        } else { */
  ...
  public String toString() {
    ...
    for (int i = 0; i < m_data.numClasses(); i++) {
      LinkedHashMap<String, Count> classDict = 
        m_probOfWordGivenClass.get(i);
