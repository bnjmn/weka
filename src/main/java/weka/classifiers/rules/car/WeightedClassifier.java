...
public class WeightedClassifier extends CarClassifier 
  ...
  public static final Tag [] TAGS_SELECTION = {
    new Tag(EQUAL, "Equal"),
    new Tag(INVERSE, "Inverse"),
    new Tag(LINEAR, "Linear")
  };
        ...
        do{
          ...
          if(!instance.isMissing(attNum) && start.equals(attNum,attValue)){
            ...
            for(int j = 0;j < (start.getContent()).size(); j++){
              ...
              temp[classAtt] = temp[classAtt]+  weight(getWeightScheme().getSelectedTag().getReadable(), ((Double)((FastVector)(start.getContent()).elementAt(j)).elementAt(3)).doubleValue());
              pathOk = true;
            }
          }
          index = index +2;
  ...
  public String toString(){
    ...
    if(getWeightScheme().getSelectedTag().getReadable().equalsIgnoreCase("inverse"))
      ...
      if(getWeightScheme().getSelectedTag().getReadable().equalsIgnoreCase("linear"))
        text.append("linear\n");
      else
