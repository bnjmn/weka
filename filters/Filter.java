...
public abstract class Filter
  ...
  /**
   * generates the header for the source code, based on the given data
   * 
   * @param data the data the header is generated for
   * @return the header
   */
  protected static String generateSourceCodeHeader(Instances data) {
    StringBuffer	result;
    ...
    result.append("// Relation: " + data.relationName() + "\n");
    result.append("\n");
  ...
  public static void filterFile(Filter filter, String [] options) 
    ...
    if (sourceCode.length() != 0)
      System.out.println(
	  generateSourceCodeHeader(data)
	  + ((Sourcable) filter).toSource(sourceCode, data));
  }
  ...
  public static void batchFilterFile(Filter filter, String [] options) 
    ...
    if (sourceCode.length() != 0)
      System.out.println(
	  generateSourceCodeHeader(firstData)
	  + ((Sourcable) filter).toSource(sourceCode, firstData));
  }
