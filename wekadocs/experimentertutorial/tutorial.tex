...
One can add dataset files either with an absolute path or with a relative one. The latter makes it often easier to experiments on different machines, hence one should check \textit{Use relative paths}, before clicking on \textit{Add new...}.
...
When the Experimenter is started in \textit{Advanced} mode, the \textit{Setup} panel is displayed. Click \textit{New} to initialize an experiment. This causes default parameters to be defined for the experiment.
\begin{center}
...
\begin{itemize}
   \item First of all select the \textit{Advanced} mode in the \textit{Setup} panel
   ...
   \item Check your results in the \textit{Analyse} panel bu clicking either the \textit{Database} or \textit{Experiment} buttons 
\end{itemize}
