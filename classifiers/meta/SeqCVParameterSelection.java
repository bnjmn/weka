...
import weka.classifiers.SequentialEvaluation;
import weka.classifiers.Classifier;
import weka.classifiers.SequentialClassifier;
...
/**
 * Class for performing parameter selection by cross-validation for a
 * sequential classifier. For more information, see<p>
 *
 ...
 */
public class SeqCVParameterSelection extends SequentialClassifier 
  ...
  protected SequentialClassifier m_Classifier = new weka.classifiers.meta.RSW();
  ...
  protected void findParamsByCrossValidation(int depth, Instances trainData)
	...
	System.err.println("");
      ...
      if(m_Classifier instanceof SequentialClassifier)	{
      	SequentialEvaluation evaluation = new SequentialEvaluation(trainData);
	
	for (int j = 0; j < m_NumFolds; j++) {
	  Instances train = trainData.seqTrainCV(m_NumFolds, j);
	  Instances test = trainData.seqTestCV(m_NumFolds, j);
	  m_Classifier.buildClassifier(train);
	  evaluation.setPriors(train);
	  evaluation.evaluateModel((SequentialClassifier)m_Classifier, test);
        }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    setClassifier((SequentialClassifier)Classifier.forName(classifierName,
  ...
  public void buildClassifier(Instances instances) throws Exception {
    ...
    if(!(m_Classifier instanceof SequentialClassifier)) {	
      ...
      m_TrainFoldSize = trainData.seqTrainCV(m_NumFolds, 0).numInstances(); 
    }  
...
*/
  /**
   * Predicts the class value for the given test sequence.
   *
   * @param instances the sequence to be classified
   * @return the predicted class value
   * @exception Exception if an error occurred during the prediction
   */
  public double [] classifySequence(Instances instances) throws Exception {
     return m_Classifier.classifySequence(instances);
  }
   ...
   */
  public void setClassifier(SequentialClassifier newClassifier) {
  ...
  public static void main(String [] argv) {
    try {
        System.out.println(SequentialEvaluation.evaluateModel(new SeqCVParameterSelection(), 
						  argv));
    } catch (Exception e) {
