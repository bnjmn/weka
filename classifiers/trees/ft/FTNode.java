...
public class FTNode extends FTtree {
   ...
   */
  public FTNode(ModelSelection modelSelection, int numBoostingIterations,
  ...
  public void buildTree(Instances data,
    ...
    double higherNumParameters) throws Exception {
    //save some stuff
    ...
    if (higherRegressions != null) {
      m_higherRegressions = higherRegressions;
    } else {
    ...
    for (j = 0; j < data.numInstances(); j++) {
      ...
      if (data.instance(j).classValue() != getConstError(probsConst)) {
        m_constError = m_constError + 1;
      }
    ...
    if (grow) {
      ...
      if (m_modelSelection instanceof BinC45ModelSelection) {
        if ((((BinC45Split) m_localModel).attIndex() >= 0) &&
              (((BinC45Split) m_localModel).attIndex() < data.classAttribute()
                                                                 .numValues())) {
          m_hasConstr = true;
        }
      } else {
        if ((((C45Split) m_localModel).attIndex() >= 0) &&
              (((C45Split) m_localModel).attIndex() < data.classAttribute()
                                                              .numValues())) {
          m_hasConstr = true;
        }
      }
      ...
      for (int i = 0; i < m_sons.length; i++) {
        m_sons[i] = new FTNode(m_modelSelection, m_fixedNumIterations,
            m_minNumInstances, getWeightTrimBeta(), getUseAIC());
        m_sons[i].buildTree(localInstances[i],
          mergeArrays(m_regressions, m_higherRegressions),
          m_totalInstanceWeight, m_numParameters);
  ...
  public double prune() throws Exception {
    ...
    if (m_isLeaf) {
      ...
      for (i = 0; i < m_sons.length; i++) {
        probBranch = m_localModel.distribution().perBag(i) / m_localModel.distribution()
                                                                         .total();
      ...
      if (Utils.smOrEq(errorsLeaf, errorsTree) &&
            Utils.smOrEq(errorsLeaf, errorsConstModel)) {
        // Free son Trees
        m_sons = null;
        m_isLeaf = true;
        m_hasConstr = false;
  ...
  public double[] distributionForInstance(Instance instance)
    ...
    if (m_isLeaf && m_hasConstr) { //leaf
      ...
      if (m_isLeaf && !m_hasConstr) {
        ...
        Instance instanceSplit = new Instance(instance.numAttributes() +
            instance.numClasses());
        instanceSplit.setDataset(instance.dataset());

        // Inserts attribute and their value
        for (int i = 0; i < instance.numClasses(); i++) {
          instanceSplit.dataset()
                       .insertAttributeAt(new Attribute("N" +
              (instance.numClasses() - i)), 0);
