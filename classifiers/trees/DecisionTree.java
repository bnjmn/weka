...
/**
 * Class for generating a reduced error pruning decision tree.
 *
 * Valid options are: <p>
 *
 * -M number <br>
 * Set minimum number of instances per leaf. (Default: 1) <p>
 *		    
 * -N number <br>
 * Number of folds for reduced error pruning. (Default: 3) <p>
 *
 * -S number <br> 
 * Seed for random data shuffling. <p>
 *
 * -P <br>
 * No pruning. <p>
 *
 * -D <br>
 * Turns debugging info on. <p>
 *
 * @author Eibe Frank (eibe@cs.waikato.ac.nz)
 * @version $Revision: 1.1 $
 */
public class DecisionTree extends DistributionClassifier 
  ...
  /** Number of pruning instances */
  private double m_NumPruningInsts = 0;
  ...
  private void buildTree(int[][] sortedIndices, double[][] weights,
    ...
    for (int i = 0; i < m_Distribution.length; i++) {
      if (Utils.grOrEq(Utils.sum(m_Distribution[i]), m_MinNum)) {
	count++;
      }
      if (count > 1) {
	break;
	...
	new double[m_Distribution.length][data.numAttributes()][0];
      ...
      m_Successors = new DecisionTree[m_Distribution.length];
      for (int i = 0; i < m_Distribution.length; i++) {
	m_Successors[i] = new DecisionTree();
  ...
  public static void main(String[] argv) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new DecisionTree(), argv));
    } catch (Exception e) {
      e.printStackTrace();
