 ...
 * a) all rules are weighted equally with weight one (default behaviour).
 * b) all rules are weighted linearly
 * c) all rules are weighted using the inverse function 1/position in sort order of mining algorithm
 * 
 * The pruning step is omitted. All mined rules are used for classifcation, if not specified otherwise with the -L option.
 *
 * Valid options are:<p>
 *
 ...
 * Sets the weighting scheme: inverse|linear|equal (default).<p>
 *
 *
 * -L rule limit<br>
 * Set a rule limit for the number of rules used for classification after the pruning step. Uses the first N rules only.<p> 
 *
 * 
...
public class WeightedClassifier extends CarClassifier implements OptionHandler, AdditionalMeasureProducer {
    ...
    private PrecedencePruning m_prune = new weka.associations.classification.PrecedencePruning();

    /** The mining algorithm. */
    private CARuleMiner m_assoc = new weka.associations.classification.CarApriori();
    ...
    /** The limit for the number of classification rules after pruning. -1 indicates no limit. Valid integers are >= 1.*/ 
    ...
    /** The sum of the rank of the first rule that covers an instance and predicts it correctly. */
   ...
   */
	...
	options[current++] = "" + m_numRules;
        ...
        options[current++] = "-W";
	options[current++] = "" + m_weightScheme;
  ...
  public void buildClassifier(Instances newInstances) throws Exception {
      ...
      Instances instances = Filter.useFilter(newInstances,m_filter);
      ...
      if(m_numRules > 0 && allRules[0].size() > m_numRules){
        ...
        for(int i =0; i< m_stop; i++){
            limRules1.addElement(allRules[0].elementAt(i));
            limRules2.addElement(allRules[1].elementAt(i));
            limRules3.addElement(allRules[2].elementAt(i));
        }
  ...
  /**
    ...
    public double[] distributionForInstance(Instance newInstance) throws Exception{
        ...
        Instance instance = m_filter.output();
        FastVector treeNodes, classify = sortAttributes(m_instances,instance, pruneTree);
        ...
        out:
        while(k < list.size() && !firstFound && size > 0){
        ...
        if(m_averageCorrect == 0){
            if ((int)instance.value(instance.classIndex()) == m_prune.getDefaultClass())
        ...
        double sum = 0;
        double[] classDistribution = new double[m_instances.numClasses()], temp = new double[m_instances.numClasses()];;
	CrNode start;
	    ...
	    treeNodes.removeElementAt(0);
            ...
            do{
                if((start.getNextSibling()).getTreeParent()==null)
                    treeNodes.addElement((CrNode)start.getNextSibling());
                    ...
                    if(!instance.isMissing(attNum) && start.equals(attNum,attValue)){
                        checkEquals = true;
                            for(int j = 0;j < (start.getContent()).size(); j++){
                                int classAtt = ((Integer)((FastVector)(start.getContent()).elementAt(j)).firstElement()).intValue();
                                temp[classAtt] = temp[classAtt]+  weight(m_weightScheme, ((Double)((FastVector)(start.getContent()).elementAt(j)).elementAt(3)).doubleValue());
                                pathOk = true;
                            }
                    }
                    index = index +2;
     ...
     */    
    public void setWeightScheme(String type){
    ...
    public double weight(String type, double rank){
        ...
        if(type.equalsIgnoreCase("linear"))
            return 1 - ((1.0/(double)(m_stop + 1)) * rank);
        return 1;
    }
    ...
    public String toString(){
        ...
        if(m_weightScheme.equalsIgnoreCase("inverse"))
            ...
            if(m_weightScheme.equalsIgnoreCase("linear"))
                text.append("linear\n");
            else
    ...
    public double getMeasure(String additionalMeasureName) {
      ...
      } else if (additionalMeasureName.compareTo("measureNumRulesAfterAddPruning") == 0) {
      return measureNumPrunedRules();
      } else if (additionalMeasureName.compareTo("measureNumClassRules") == 0) {
      return measureNumClassRules();
      } else if (additionalMeasureName.compareTo("measureAverageRankFirstRuleFires") == 0) {
      return measureAverageRankFires();
      } else if (additionalMeasureName.compareTo("measureAverageRankFirstCorrectRule") == 0) {
   ...
   */  
     ...
     * @return the average rank of the first rule that covers an instance and predicts it correctly.
     */    
  ...
  public static void main(String [] argv) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new WeightedClassifier(), argv));
    } catch (Exception e) {
        e.printStackTrace();
