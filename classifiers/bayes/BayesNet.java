 ...
 */
public class BayesNet extends DistributionClassifier 
...
//		m_Instances = new Instances(m_Instances, 0);
	...
	public void initStructure() throws Exception {
		...
		if (m_bInitAsNaiveBayes) {
			...
			for (int iOrder = 1; iOrder < m_Instances.numAttributes(); iOrder++) {
				...
				m_ParentSets[iAttribute].AddParent(m_Instances.classIndex(), m_Instances);
			}
		}
	}
	...
	*/
	public void estimateCPTs() throws Exception {
		...
		for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
			...
			{
				m_Distributions[iAttribute][iParent] = new DiscreteEstimatorBayes(m_Instances.attribute(iAttribute).numValues(), m_fAlpha);
			}
		}
		// Compute counts
					...
					if (nParent == m_Instances.classIndex()) {
						...
						iCPT = iCPT * m_Instances.attribute(nParent).numValues() + instance.value(nParent);
                                ...
                                if (iAttribute == m_Instances.classIndex()) {
                                    fP *= m_Distributions[iAttribute][(int) iCPT].getProbability(iClass);
                                } else {
                                    fP *= m_Distributions[iAttribute][(int) iCPT].getProbability(instance.value(iAttribute));
                                }
			}
        		fProbs[iClass] *= fP;
		}
  ...
  public Enumeration listOptions() {
    ...
    newVector.addElement(new Option("\tScore type (BAYES, MDL, ENTROPY, or AIC)\n", "S", 1,"-S [BAYES|MDL|ENTROPY|AIC]"));
  ...
  public String [] getOptions() {
    ...
    int current = 0;
		...
		if (m_nMaxNrOfParents != 10000) {
			options[current++] = "-P"; options[current++] = "" + m_nMaxNrOfParents;
		}
	...
	*/
	public double logScore(int nType) {
		...
		for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
			for (int iParent = 0; iParent < m_ParentSets[iAttribute].GetCardinalityOfParents(); iParent++)
			{
				fLogScore += ((Scoreable) m_Distributions[iAttribute][iParent]).logScore(nType);
			}
			switch (nType) {
			case(Scoreable.MDL): {
				fLogScore -= 0.5 * 
					m_ParentSets[iAttribute].GetCardinalityOfParents() * (m_Instances.attribute(iAttribute).numValues() - 1) *
...
//                        text.append("\n");
	...
	**/
	protected double CalcScoreWithExtraParent(int nNode, int nCandidateParent) {
		...
		int nCardinality = m_ParentSets[nNode].GetCardinalityOfParents() * m_Instances.attribute(nCandidateParent).numValues();
			...
			switch (m_nScoreType) {
				case (Scoreable.BAYES): {
					...
					for(int iSymbol = 0; iSymbol < numValues; iSymbol++) {
						...
						fLogScore += SpecialFunctions.lnGamma(m_fAlpha + nCounts[iParent][iSymbol]);
