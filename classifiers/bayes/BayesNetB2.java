...
//		double fBestScore = CalcNodeScore(iAttribute);
		...
		while (bProgress && m_ParentSets[iAttribute].GetNrOfParents() < m_nMaxNrOfClassifierParents) {
			int nBestAttribute = -1;
			for (int iAttribute2 = 0; iAttribute2 < m_Instances.numAttributes(); iAttribute2++) {
				if (iAttribute != iAttribute2) {
					double fScore = CalcScoreWithExtraParent(iAttribute, iAttribute2);
					if (fScore > fBestScore) {
						fBestScore = fScore;
				...
				if (!bDone[iNode2]) {
					...
					for (int iParent = 0; iParent < m_ParentSets[iNode2].GetNrOfParents(); iParent++) {
						if (!bDone[m_ParentSets[iNode2].GetParent(iParent)]) {
							...
							if (iNode2 != iAttributeHead || m_ParentSets[iNode2].GetParent(iParent) != iAttributeTail) {
								bHasNoParents = false;
							}
						}
					}
