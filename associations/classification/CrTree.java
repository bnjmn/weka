 ...
 * It is needed to implement a CrTree as described at:
 * W. Li, J. Han, J.Pei: CMAR: Accurate and Efficient Classification Based on Multiple
 * Class-Association Rules. In ICDM'01:369-376,2001.
 *
 * This class allows items to be sorted according to their frequency.
 ...
 */
	...
	public FrequencyObjects(int num, int value, int freq){
        ...
        /**
         * Compares two items according to their frequency
         * Note: this class has a natural ordering that is inconsistent with equals
         * @param obj the object that has to be compared
         * @return 0, if their frequencies are equak
         * 1, if the frequency of the actual item is greater
         * -1, otherwise
         */        
	public int compareTo(Object obj){
         ...
         * @return true if the two items are identical, e.g. they have the same attribute and attribute value
         */        
	public  boolean equals(FrequencyObjects obj){
	
	if((obj.m_attributeNumber==m_attributeNumber) && (obj.m_attributeValue==m_attributeValue))
	    return true;
	return false;
	}

        /**
         * Returns a string conatining attribute number, attribute value and frequency in the dataset.
     ...
     * The default class is stored in the head of the associated CrList.
     */    
    public void setDefaultClass(){
	
	int[] defaultClass = new int[2], classes = new int[(m_class.attribute(0)).numValues()];
	defaultClass[0] = 0;
     ...
     * For every item the resulting FastVector conatins two integer: attribute number and value
	...
	for(int j = 0;j < m_sortedFrequencies.size();j++){
	    if(current.itemAt(((FrequencyObjects)(m_sortedFrequencies.elementAt(j))).m_attributeNumber)
	       == ((FrequencyObjects)(m_sortedFrequencies.elementAt(j))).m_attributeValue){
		sortedPremise.addElement(new Integer(((FrequencyObjects)(m_sortedFrequencies.elementAt(j))).m_attributeNumber));
		sortedPremise.addElement(new Integer(((FrequencyObjects)(m_sortedFrequencies.elementAt(j))).m_attributeValue));
	    }
	}
     ...
     * This method is provided for the children of CrTree.
     * This class implements no pruning and therefore needs no additional pruning criterias
     ...
     * In its general form here, no pruning is done.
     ...
     * If no other node stores the same item, the element in the associated list is deleted as well.
     * @param node the node that has to be deleted
     */    
	...
	if(node.getListSibling()==null){
		...
		(node.getLastListSibling()).setNextListSibling(node.getNextListSibling());
		(node.getNextListSibling()).setLastListSibling(node.getLastListSibling());
		node.setLastListSibling(null);
    ...
    /**
     * insert a new rule into the CrTree in three steps:
     * 1. search for existing paths and the right node to start the insertion.
     * 2. insert (rest of) premise
     * 3. insert consequence and additional measures (by calling insertContent)
     * @param rulePremise the rule premise
     * @param ruleConsequence the consequence and additional measures
     */    
    public final void insertNode(FastVector rulePremise, FastVector ruleConsequence){
	...
	while(remainder.size()>0){
	    ...
	    CrListElement contained = m_list.searchListElement(pathInfo[0],pathInfo[1]);
	    int height = 1+ ((premiseSize - remainder.size())/2);
            
            //item nowhere in the CrTree
	    if(contained==null){
                //new list entry
		CrListElement list = m_list.insertListElement(pathInfo[0],pathInfo[1],height);
		//new node
     ...
     * In thgis general form of a CrTree there is no pruning involved.
     * Children of this class implement this method with their pruning strategy.
     */    
    public void prune(){
	...
	if((subTreeRoot.getContent()).size()!=0){
		for(int i = 0;i < (subTreeRoot.getContent()).size();i++){
		    FastVector actual = (FastVector)(subTreeRoot.getContent()).elementAt(i);
	...
	while(m_storeSiblings.size()!=0){
		...
		m_storeSiblings.addElement((CrNode)m_actual.getNextSibling());
	    if(((FastVector)m_actual.getContent()).size()!=0){
		for(int i = 0;i < (m_actual.getContent()).size(); i++){
		    FastVector actual = (FastVector)(m_actual.getContent()).elementAt(i);
		    result[((Integer)actual.firstElement()).intValue()]++;
		}
	    }
	    m_actual = m_actual.getTreeChild();
    ...
    /**
     * Prints a rule premise (from bottom to top in the tree,
     * respectively infrequent items to frequent ones).
	...
	do{
	    ...
	    pathText.append(m_instances.attribute(info[0]).value(info[1])+' '+info[0]+' '+info[1]+' ');
	...
	do{
		...
		for(int k = 0;k < (m_actual.getContent()).size(); k++){
		    ...
		    text.append(premise);
		    text.append(" ==> ");
		    FastVector actualClass = (FastVector)((FastVector)m_actual.getContent()).elementAt(k);
		    int classInfo = ((Integer)actualClass.firstElement()).intValue();
		    text.append(m_class.attribute(0).name()+'=');
		    text.append(m_class.attribute(0).value(classInfo)+"     "+metricString+":");
