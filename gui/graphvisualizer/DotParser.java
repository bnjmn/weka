...
public class DotParser implements GraphConstants  {
   ...
   protected void setSyntax(StreamTokenizer tk) {
	...
	if(tk.sval.equalsIgnoreCase("graph") || tk.sval.equalsIgnoreCase("node") || tk.sval.equalsIgnoreCase("edge") )
	    ...
	    catch(Exception ex) {  System.err.println("error at lineno "+tk.lineno()+" in stmtException"); ex.printStackTrace();
	    }
	}
    }
	...
	if(tk.ttype==']' || tk.ttype==tk.TT_EOF) 
		...
		if(tk.ttype=='=') {
			...
			{ System.err.println("couldn't find label at line "+tk.lineno()); tk.pushBack();}
		}
		else 
		    { System.err.println("couldn't find label at line "+tk.lineno()); tk.pushBack(); }
	    }
		...
		if(tk.ttype=='=') {
			...
			{ System.err.println("couldn't find color at line "+tk.lineno()); tk.pushBack(); }
		}
		else 
		    { System.err.println("couldn't find color at line "+tk.lineno()); tk.pushBack(); }
	    }
		...
		if(tk.ttype=='=') {
			...
			{ System.err.println("couldn't find style at line "+tk.lineno()); tk.pushBack(); }
		}	    
		else 
		    { System.err.println("couldn't find style at line "+tk.lineno()); tk.pushBack(); }
	    }
	...
	if(tk.ttype=='>') {
		...
		while(true) {
			...
			if( m_edges!=null && !(m_edges.contains(e)) ) {
			    ...
			    //System.out.println("Added edge from "+((GraphNode)(m_nodes.elementAt(nindex))).ID+" to "+
