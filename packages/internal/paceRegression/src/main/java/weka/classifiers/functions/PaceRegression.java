...
/**
 * <!-- globalinfo-start --> Class for building pace regression linear models
 * and using them for prediction. <br/>
 * <br/>
 * Under regularity conditions, pace regression is provably optimal when the
 * number of coefficients tends to infinity. It consists of a group of
 ...
 * @version $Revision$
 */
public class PaceRegression extends AbstractClassifier implements
  ...
  public static final Tag[] TAGS_ESTIMATOR = {
    ...
    new Tag(pace2Estimator, "PACE2"), new Tag(pace4Estimator, "PACE4"),
    new Tag(pace6Estimator, "PACE6"),
    new Tag(olscEstimator, "Ordinary least squares selection"),
    new Tag(aicEstimator, "AIC"), new Tag(bicEstimator, "BIC"),
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    double[][] transformedDataMatrix = getTransformedDataMatrix(data,
      m_ClassIndex);
  ...
  private double[] pace(double[][] matrix_X, double[] vector_Y) {
    ...
    case ricEstimator:
    case olscEstimator:
      if (paceEstimator == aicEstimator) {
        olscThreshold = 2;
      } else if (paceEstimator == bicEstimator) {
        olscThreshold = Math.log(n);
      } else if (paceEstimator == ricEstimator) {
        olscThreshold = 2 * Math.log(kr);
      }
      aTilde = aHat.copy();
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (estimator.equals("ols")) {
      paceEstimator = olsEstimator;
    } else if (estimator.equals("olsc")) {
      paceEstimator = olscEstimator;
    } else if (estimator.equals("eb") || estimator.equals("")) {
      paceEstimator = ebEstimator;
    } else if (estimator.equals("nested")) {
      paceEstimator = nestedEstimator;
    } else if (estimator.equals("subset")) {
      paceEstimator = subsetEstimator;
    } else if (estimator.equals("pace2")) {
      paceEstimator = pace2Estimator;
    } else if (estimator.equals("pace4")) {
      paceEstimator = pace4Estimator;
    } else if (estimator.equals("pace6")) {
      paceEstimator = pace6Estimator;
    } else if (estimator.equals("aic")) {
      paceEstimator = aicEstimator;
    } else if (estimator.equals("bic")) {
      paceEstimator = bicEstimator;
    } else if (estimator.equals("ric")) {
      paceEstimator = ricEstimator;
    } else {
      throw new WekaException("unknown estimator " + estimator
        + " for -E option");
    }

    String string = Utils.getOption('S', options);
    if (!string.equals("")) {
      olscThreshold = Double.parseDouble(string);
    }
