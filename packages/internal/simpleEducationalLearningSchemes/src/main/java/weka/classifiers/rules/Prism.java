...
/**
 * <!-- globalinfo-start --> Class for building and using a PRISM rule set for
 * classification. Can only deal with nominal attributes. Can't deal with
 * missing values. Doesn't do any pruning.<br/>
 * <br/>
 ...
 * @version $Revision$
 */
public class Prism extends AbstractClassifier implements
  ...
  private class PrismRule implements Serializable, RevisionHandler {
    ...
    /** The classification */
    private final int m_classification;
    ...
    public PrismRule(Instances data, int cl) throws Exception {
      ...
      while (enu.hasMoreElements()) {
        if ((int) enu.nextElement().classValue() != cl) {
          m_errors++;
        }
      }
    ...
    public String toString() {
      ...
      try {
        ...
        text
          .append(m_instances.classAttribute().value(m_classification) + "\n");
        if (m_next != null) {
   ...
   */
  private class Test implements Serializable, RevisionHandler {
      ...
      while (contains(E, cl)) { // while E contains examples in class cl
        ...
        while (rule.m_errors != 0) { // until the rule is perfect
          ...
          while (enumAtt.hasMoreElements()) {
            Attribute attr = enumAtt.nextElement();
            ...
            Enumeration<Instance> enu = ruleE.enumerateInstances();
            while (enu.hasMoreElements()) {
              Instance i = enu.nextElement();
            ...
            for (int val = 0; val < M; val++) {
              ...
              if (test.m_attr == -1 || diff > 0
                || (diff == 0 && correct[val] > bestCorrect)) {
  ...
  private static boolean contains(Instances E, int C) throws Exception {
    ...
    Enumeration<Instance> enu = E.enumerateInstances();
    while (enu.hasMoreElements()) {
      if ((int) enu.nextElement().classValue() == C) {
        return true;
      }
    }
    return false;
