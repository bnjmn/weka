...
/**
 * <!-- globalinfo-start --> A wrapper class for Jython code. Even though the
 ...
 * href="http://wiki.python.org/jython/JythonMonthly/Articles/September2006/1"
 * target="_blank">this</a> code.
 * 
 * @author FracPete (fracpete at waikato dot ac dot nz)
 * @version $Revision$
 */
public class JythonClassifier extends AbstractClassifier {
  ...
  public String globalInfo() {
    return "A wrapper class for Jython code. Even though the classifier is "
  ...
  public Enumeration<Option> listOptions() {
    ...
    result.addElement(new Option("\tThe Jython module to load (full path)\n"
      + "\tOptions after '--' will be passed on to the Jython module.", "J", 1,
      "-J <filename>"));

    result.addElement(new Option(
      "\tThe paths to add to 'sys.path' (comma-separated list).", "P", 1,
      "-P <directory[,directory,...]>"));
  ...
  public String[] getOptions() {
    ...
    if (m_JythonOptions.length > 0) {
      String[] options = m_JythonOptions;
      result.add("--");
  ...
  protected void initJythonObject() {
    try {
      if (m_JythonModule.isFile()) {
        m_JythonObject = (Classifier) Jython.newInstance(m_JythonModule,
          Classifier.class, m_JythonPaths);
      } else {
        m_JythonObject = null;
      }
