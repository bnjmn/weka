 ...
 * @author Tim Leathart
 */
public class PCTNodeBase extends AbstractClassifier implements
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        if (m_fixedNumIterations > 0) {
           ...
           throw new IllegalArgumentException("Option not available.");
        }
    ...
    protected int performBoosting(Instances train, Instances test,
        ...
        error[0] += getRootMeanSquaredError(test);
        ...
        while (iteration < maxIterations) {
            ...
            error[iteration] += getRootMeanSquaredError(test);
     ...
     * @throws Exception if something goes wrong
     */
    protected double getRootMeanSquaredError(Instances data) throws Exception {
    ...
    protected boolean performIteration(int iteration, double[][] trainYs,
        ...
        for (int j = 0; j < m_numClasses; j++) {
            ...
            Instances instancesCopy = new Instances(trainNumeric, trainNumeric.numInstances());

            if (weightSum > 0) {
                if (m_weightTrimBeta <= 0) {
                    for (int i = 0; i < trainNumeric.numInstances(); i++) {
                        double[] instValues = trainNumeric.instance(i).toDoubleArray();
                        double weight = trainNumeric.instance(i).weight();

                        Instance newInstance = new DenseInstance(weight, instValues);
                        instancesCopy.add(newInstance);
                    }
                } else {
                    // Get weights
                    ...
                    for (int i = weightsOrder.length - 1; (i >= 0)
                        ...
                        double[] instValues = trainNumeric.instance(weightsOrder[i]).toDoubleArray();

                        double weight = weights[weightsOrder[i]];
                        Instance newInstance = new DenseInstance(weight, instValues);
        ...
        for (int i = 0; i < m_numClasses; i++) {
            SimpleLinearRegression slr = linearRegressionForEachClass[i];
            SimpleLinearRegression newSlr = new SimpleLinearRegression(slr.getAttributeIndex(),
                    slr.getSlope(), slr.getIntercept());

            m_regressions[i][newSlr.getAttributeIndex()].addModel(newSlr);
        }
    ...
    protected double[] getFs(Instance instance) throws Exception {
        ...
        for (int i = 0; i < m_numericDataHeader.numAttributes(); i++) {
            if (i != m_numericDataHeader.classIndex()) {
                ...
                // get the score for each class for this attribute
                for (int j = 0; j < m_numClasses; j++) {
