...
/**
 *  Provides root mean square logarithmic error for evaluating regression
 *  schemes.
 *
 * @author Eibe Frank (eibe@cs.waikato.ac.nz)
 * @version $Revision: 11192 $
 */
public class RootMeanSquareLogarithmicError extends AbstractEvaluationMetric implements StandardEvaluationMetric {

  /** Sum of square log errors */
  protected double m_SumSquareLogErrors = 0;
   ...
   */
  public String getMetricDescription() { return "The root mean square logarithmic error, using log(x + 1)."; }
  ...
  public  void updateStatsForPredictor(double predictedValue, Instance instance) {
    ...
    if (!instance.classIsMissing()) {
      if (!Utils.isMissingValue(predictedValue)) {
        ...
        m_SumSquareLogErrors += instance.weight() * diff * diff;
  ...
  public String toSummaryString() {
    ...
    return "Root mean square logarithmic error " + Utils.doubleToString(getStatistic("rmsle"), 12, 4) + "\n";
  }
  ...
  public double getStatistic(String name) {
    ...
    if (!name.equals("rmsle")) {
      throw new UnknownStatisticException("Statistic " + name + " is unknown.");
    }

    return Math.sqrt(m_SumSquareLogErrors / m_SumOfWeights);
  }
