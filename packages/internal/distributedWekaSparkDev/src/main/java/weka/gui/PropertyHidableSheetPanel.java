 ...
 */
public class PropertyHidableSheetPanel extends JPanel implements
  ...
  /**
   * Whether to use EnvironmentField and FileEnvironmentField for text and file
   * properties respectively
   */
  private boolean m_useEnvironmentPropertyEditors;

  /** Any user hidden properties */
  protected List<String> m_userHiddenProperties = new ArrayList<>();

  /**
   * Creates the property sheet panel with an about panel.
   */
  public PropertyHidableSheetPanel() {
   ...
   */
  public PropertyHidableSheetPanel(boolean showAboutPanel) {
    super();
    m_showAboutPanel = showAboutPanel;
  }

  /**
   * Set whether to use environment property editors for string and file
   * properties
   *
  ...
  /**
   * Get whether to use environment property editors for string and file
   * properties
   *
  ...
  /**
   * Set any user hidden properties
   *
   * @param userHiddenProperties the list of properties to hide from the user
   */
  public void setUserHiddenProperties(List<String> userHiddenProperties) {
    m_userHiddenProperties = userHiddenProperties;
  }

  /**
   * Get any user hidden properties
   *
   * @return the list of properties to hide from the user
   */
  public List<String> getUserHiddenProperties() {
    return m_userHiddenProperties;
  }
  ...
  public synchronized void setTarget(Object targ) {
    ...
    try {
      ...
      custClass =
        Introspector.getBeanInfo(m_Target.getClass()).getBeanDescriptor()
          .getCustomizerClass();
    ...
    for (MethodDescriptor m_Method : m_Methods) {
      ...
      if (name.endsWith("TipText") || o != null) {
        if (meth.getReturnType().equals(String.class) || o != null) {
          try {
            String tempTip =
              o != null ? o.description() : (String) (meth.invoke(m_Target,
                args));
            ...
            tempTip =
              tempTip.replace("<html>", "").replace("</html>", "")
                .replace("<br>", "\n").replace("<p>", "\n\n");
    ...
    for (int i = 0; i < m_Properties.length; i++) {
      ...
      if (m_userHiddenProperties.contains(m_Properties[sortedPropOrderings[i]]
        .getName())) {
        // skip user-hidden properties
        continue;
      }
      ...
      try {
        ...
        Class<?> pec =
          m_Properties[sortedPropOrderings[i]].getPropertyEditorClass();
        if (pec != null) {
          try {
        ...
        if (editor == null) {
          ...
          } else if (m_useEnvironmentPropertyEditors
            && String.class.isAssignableFrom(type)) {
            editor = new EnvironmentField();
          } else if ((m_useEnvironmentPropertyEditors || fileProp != null)
            && File.class.isAssignableFrom(type)) {
            if (fileProp != null) {
              editor =
                new FileEnvironmentField("", fileProp.fileChooserDialogType(),
                  fileProp.directoriesOnly());
            } else {
              ...
              if (meth.getReturnType().equals(String.class)) {
                try {
                  ...
                  if (ci < 0) {
                    ...
                    m_TipTexts[sortedPropOrderings[i]] =
                      tempTip.substring(0, ci);
                  }
                  /*
      ...
      m_Labels[sortedPropOrderings[i]].setBorder(BorderFactory
        .createEmptyBorder(10, 10, 0, 5));
      ...
      if (m_TipTexts[sortedPropOrderings[i]] != null) {
        m_Views[sortedPropOrderings[i]]
          .setToolTipText(m_TipTexts[sortedPropOrderings[i]]);
        m_Labels[sortedPropOrderings[i]]
          .setToolTipText(m_TipTexts[sortedPropOrderings[i]]);
      }
    ...
    if (m_NumEditable == 0) {
      JLabel empty =
        new JLabel("No editable properties", SwingConstants.CENTER);
  ...
  protected void openHelpFrame() {
    ...
    if (PropertyDialog.getParentDialog(this) != null) {
      ...
      jdtmp =
        new JDialog(PropertyDialog.getParentDialog(m_aboutPanel), "Information");
    }
  ...
  protected void openCapabilitiesHelpDialog() {
    if (PropertyDialog.getParentDialog(this) != null) {
      m_CapabilitiesDialog =
        new CapabilitiesHelpDialog(PropertyDialog.getParentDialog(this));
    } else {
      m_CapabilitiesDialog =
        new CapabilitiesHelpDialog(PropertyDialog.getParentFrame(this));
    }
