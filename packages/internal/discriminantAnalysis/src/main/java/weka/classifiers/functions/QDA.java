 ...
 */
public class QDA extends AbstractClassifier {
  
  /** for serialization */
  static final long serialVersionUID = -9113383698193689291L;
  ...
  public void buildClassifier(Instances insts) throws Exception {
    ...
    // Normalize arrays of weights
    for (int i = 0; i < insts.numClasses(); i++) {
      try {
        Utils.normalize(weights[i]);
      } catch (Exception e) {
        System.err.println("Warning: couldn't normalize instance weights for class, skipping class " +
            insts.classAttribute().value(i));
        weights[i] = null;
      }
    }
