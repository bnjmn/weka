...
import weka.distributed.spark.WDDataset;
...
public class MLlibClassifierEvaluationSparkJob extends BaseStep {
   ...
   */
  protected transient List<Map.Entry<String, WDDataset>> m_currentDatasets;
  ...
  protected void runJob() throws WekaException {
    ...
    try {
      ...
      if (m_currentDatasets != null) {
        for (Map.Entry<String, WDDataset> e : m_currentDatasets) {
  ...
  protected void outputFailureData(String reason,
    Iterator<Map.Entry<String, WDDataset>> datasetIterator) throws WekaException {
    List<Map.Entry<String, WDDataset>> datasets =
      datasetIterator.hasNext() ? new ArrayList<Map.Entry<String, WDDataset>>()
        : null;
