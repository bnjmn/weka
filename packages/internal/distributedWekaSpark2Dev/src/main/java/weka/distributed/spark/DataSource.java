...
public abstract class DataSource extends SparkJob {
  ...
  public Dataset<Row> executeSQL(Dataset<Row> df, SQLContext context)
  ...
  public Dataset<Row> unionWithExisting(Dataset<Row> toUnion) {
    Dataset<Row> result = toUnion;
    if (getUnionWithExisting()) {
      Dataset<Row> existing = WDDataset.getDataFrame(this, m_datasetType.toString());
      if (existing != null) {
   ...
   */
  public static Dataset<Row> applyPartitioning(Dataset<Row> df, int minPartitions,
    int maxPartitions) {
    Dataset<Row> result = df;
  ...
  /**
   * Set the number of partitions to use in the Dataset<Row>. Does nothing if not
   * specified, or the number of requested partitions is equal to the number in
   * the Dataset<Row> already.
   *
   * @param df the {@code Dataset<Row>} to repartition/coalesce
   * @return the repartitioned {@code Dataset<Row>}
   */
  protected Dataset<Row> applyPartitioning(Dataset<Row> df) {
    int minPartitions = -1;
    int maxPartitions = -1;
  ...
  /**
   * Persist the {@code Dataset<Row>} with the configured caching strategy
   *
   * @param df the {@code Dataset<Row>} to persist
   */
  protected void persist(Dataset<Row> df) {
