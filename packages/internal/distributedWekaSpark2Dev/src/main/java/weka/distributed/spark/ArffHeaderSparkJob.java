...
public class ArffHeaderSparkJob extends SparkJob implements
   ...
   * until it is needed?
   *
   * @param frames the {@code Dataset<Row>}s to process
   * @param unpersistFrames true if any {@code Dataset<Row>} processed into RDDs
   *          of Instances should be unpersisted
   * @throws IOException if a problem occurs
   * @throws DistributedWekaException if a problem occurs
   */
  @SuppressWarnings("unchecked")
  protected void processDataFrames(List<Dataset<Row>> frames,
    ...
    Dataset<Row> mondoFrame = frames.get(0);
      ...
      JavaRDD<CSVToARFFHeaderMapTask> mapResults =
        input.mapPartitions(
          new FlatMapFunction<Iterator<Row>, CSVToARFFHeaderMapTask>() {
            ...
            @Override
            public Iterator<CSVToARFFHeaderMapTask> call(Iterator<Row> split)
    ...
    Iterator<Map.Entry<String, WDDataset>> dataSets =
    ...
    while (dataSets.hasNext()) {
      Map.Entry<String, WDDataset> nextE = dataSets.next();
      WDDataset current = nextE.getValue();
      String datasetType = nextE.getKey();
      Dataset<Row> sourceFrame = current.getDataFrame();
      if (sourceFrame != null) {
  ...
  protected void processWithDatasource(JavaSparkContext sparkContext)
    ...
    List<Dataset<Row>> dataFrames = WDDataset.getAllDataFrames(m_dataSource);
  ...
  public boolean runJobWithContext(JavaSparkContext sparkContext)
    ...
    List<Dataset<Row>> dataFrames = WDDataset.getAllDataFrames(this);
      ...
      result =
        toCombine
          .mapPartitions(new FlatMapFunction<Iterator<CSVToARFFHeaderMapTask>, CSVToARFFHeaderMapTask>() {
            ...
            @Override
            public Iterator<CSVToARFFHeaderMapTask> call(
              ...
              return m_combinedPartition.iterator();
            }
