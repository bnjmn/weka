 ...
 */
public class WDDataset extends AbstractDataset {
  ...
  protected Dataset<Row> m_backingDataFrame;
   ...
   */
  public WDDataset(JavaRDD<Instance> dataset, Instances headerWithSummary) {
   ...
   */
  public WDDataset(Dataset<Row> baseDataFrame) {
   ...
   */
  public WDDataset(JavaRDD<Instance> backingRDD, Dataset<Row> backingDataFrame,
  ...
  /**
   * Get the {@code Dataset<Row>} dataset
   *
   * @return the dataframe, or null if this WDDataset is only backed by an
   *         {@code RDD}
   */
  ...
  /**
   * Set the {@code Dataset<Row>} dataset
   *
   * @param frame the Dataset to set
   */
  public void setDataFrame(Dataset<Row> frame) {
  ...
  public static Dataset<Row> getAnyDataFrame(SparkJob job) {
    Dataset<Row> result = null;
    Iterator<Map.Entry<String, WDDataset>> e = job.getDatasetIterator();
    while (e.hasNext()) {
      WDDataset next = e.next().getValue();
  ...
  public static List<Dataset<Row>> getAllDataFrames(SparkJob job) {
    List<Dataset<Row>> dfs = new ArrayList<Dataset<Row>>();
    for (WDDataset d : job.getDatasets()) {
  ...
  public static Dataset<Row> getDataFrame(SparkJob job, String datasetType) {
    WDDataset r = job.getDataset(datasetType);
    Dataset<Row> result = null;
    if (r != null) {
   ...
   * @param frame the {@code Dataset<Row>} input
  ...
  public static JavaRDD<Instance> dataFrameToInstanceRDD(
    CachingStrategy strategy, Dataset<Row> frame, Instances header,
    CSVToARFFHeaderMapTask rowHelper, boolean makeSparseInstances,
   ...
   */
  public static WDDataset instancesToDataset(Instances toConvert,
    ...
    return new WDDataset(instanceRDD, headerWithSummary);
  }
}
