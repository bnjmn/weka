...
/**
 * DataSink for writing to CSV files from {@code Dataset<Row>}s
 *
...
public class CSVDataSink extends FileDataSink {
  ...
  public boolean runJobWithContext(JavaSparkContext sparkContext)
    ...
    Iterator<Map.Entry<String, WDDataset>> datasetIterator =
      m_datasetManager.getDatasetIterator();
    List<Dataset<Row>> dataFrames = new ArrayList<Dataset<Row>>();
    while (datasetIterator.hasNext()) {
      Map.Entry<String, WDDataset> ne = datasetIterator.next();
    ...
    // List<Dataset<Row>> dataFrames = Dataset.getAllDataset<Row>s(this);
    ...
    for (int i = 0; i < dataFrames.size(); i++) {
      Dataset<Row> df = dataFrames.get(i);
