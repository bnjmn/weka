...
public class NNConditionalEstimator implements ConditionalEstimator {
   ...
   */
  private Vector m_CondValues;

  /**
   * Vector containing the associated weights
   */
  private Vector m_Weights;
   ...
   * 2 * PI
   */
  private static double k_2PI = 2*Math.PI;
  ...
  {
    return Math.exp(-x*x / (2*variance)) / Math.sqrt(variance*k_2PI);
  }
  ...
  {
    m_CondValues = new Vector();
    m_Values = new Vector();
    m_Weights = new Vector();
    m_SumOfWeights = 0;
    m_Covariance = null;
    m_CondMean = 0;
    m_ValueMean = 0;
  ...
  public void addValue(double data, double given, double weight)
  {
	...
	|| (((Double)m_CondValues.elementAt(insertIndex)).doubleValue() != given)
	|| (((Double)m_Values.elementAt(insertIndex)).doubleValue() != data))
    {
    ...
    m_Covariance = null; // Invalidate any previously calculated covariance matrix
  }
  ...
  public Estimator getEstimator(double given)
  {
    if (m_Covariance == null)
      calculateCovariance();
    Estimator result = new MahalanobisEstimator(m_Covariance,given-m_CondMean,
						m_ValueMean);
    return result;
  }
